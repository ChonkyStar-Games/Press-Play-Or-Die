...
var frozen: bool = false  # Whether growth/shrink is frozen

# Visual effects system - allows multiple effects to be layered
var active_visual_effects = {}  # Dictionary of effect_name -> effect_data
...
# Visual effects API
func add_visual_effect(effect_name: String, effect_data: Dictionary = {}):
    active_visual_effects[effect_name] = effect_data
    queue_redraw()

func remove_visual_effect(effect_name: String):
    active_visual_effects.erase(effect_name)
    queue_redraw()

func has_visual_effect(effect_name: String) -> bool:
    return active_visual_effects.has(effect_name)
...
func _process(delta):
    ...
    # Only apply growth/shrink when not frozen
    if not frozen:
...
func _draw():
    ...
    # Generate base button shape (used by main button and effects)
    ...
    # Determine base button color (can be modified by effects)
    var button_color = Color.WHITE
    
    # Apply color modifiers from active effects (applied in order)
    for effect_name in active_visual_effects.keys():
        if effect_name == "ice":
            button_color = Color(0.7, 0.9, 1.0, 1.0)  # Light cyan/ice blue
    
    # Draw main button
    draw_colored_polygon(points, button_color)
    
    # Render all active visual effects (layered on top of base button)
    for effect_name in active_visual_effects.keys():
        var effect_data = active_visual_effects[effect_name]
        _draw_visual_effect(effect_name, effect_data, points)
    ...
    # Determine text color (can be modified by effects)
    var text_color = Color.BLACK
    if has_visual_effect("ice"):
        text_color = Color(0.1, 0.3, 0.5, 1.0)  # Dark blue for ice
    
    draw_string(font, text_pos, text, HORIZONTAL_ALIGNMENT_CENTER, -1, font_size, text_color)

# Renders a specific visual effect - extend this function to add new effects
func _draw_visual_effect(effect_name: String, effect_data: Dictionary, base_points: PackedVector2Array):
    match effect_name:
        "ice":
            _draw_ice_effect(effect_data)
        _:
            push_warning("Unknown visual effect: " + effect_name)

# Ice effect renderer
func _draw_ice_effect(_effect_data: Dictionary):
    # Draw a smaller, brighter highlight polygon for the shine effect
    var shine_points = PackedVector2Array()
    var shine_offset = Vector2(-radius * 0.15, -radius * 0.2)  # Offset shine to upper-left
    var shine_radius = radius * 0.4  # Smaller than main circle
    
    for i in range(NUM_POINTS):
        var angle = (float(i) / NUM_POINTS) * TAU
        # Use same wobble for consistency
        var current_radius = max(1.0, shine_radius + point_offsets[i] * 0.4)
        var point = Vector2(cos(angle), sin(angle)) * current_radius + shine_offset
        shine_points.append(point)
    
    # Draw bright white shine with transparency
    draw_colored_polygon(shine_points, Color(1, 1, 1, 0.4))
    
    # Draw animated sparkle highlights
    var num_sparkles = 3
    for i in range(num_sparkles):
        var sparkle_angle = (float(i) / num_sparkles) * TAU + Time.get_ticks_msec() * 0.001
        var sparkle_dist = radius * 0.6
        var sparkle_pos = Vector2(cos(sparkle_angle), sin(sparkle_angle)) * sparkle_dist
        # Draw small bright dots
        draw_circle(sparkle_pos, radius * 0.03, Color(1, 1, 1, 0.8))
...
func _input(event):
    ...
    if is_click or is_tap:
        ...
        if pos.length() <= radius:
            # Only accumulate shrink_velocity when not frozen
            if not frozen:
